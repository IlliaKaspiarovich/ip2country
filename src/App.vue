  ...
  <div class="container">
    <div class="card">
      <div class="header">
        ...
        <p class="subtitle">Enter IP addresses to resolve their location and local time. You can add multiple rows.</p>
      </div>


      <div class="toolbar">
        <button class="button" @click="addRow">Add</button>
        <button class="button secondary" :disabled="rows.length === 0" @click="clearAll">Clear</button>
      </div>
      ...
      <div class="rows">
        <div v-for="row in rows" :key="row.id" class="row">
          ...
          />
          <div>
            <div v-if="row.status === 'loading'" class="status"><span class="spinner"></span> Looking up…</div>
            ...
            <div v-else-if="row.status === 'done' && row.result" class="status">
              ...
              <div class="small" v-if="row.result.timezone">Local time: {{ timeNow(row.result.timezone) }}</div>
            </div>
      ...
      <div class="footer">Tip: Paste multiple IPs across rows and tab/blur to resolve each independently.</div>
    </div>
  </div>
</template>
...
function addRow() {
  rows.push({ id: crypto.randomUUID?.() || `${Date.now()}-${Math.random()}`, label: `IP #${counter++}`, ip: '', status: 'idle' })
}
...
async function onBlur(row: RowItem) {
  ...
  if (!value) { row.status = 'idle'; row.error = undefined; row.result = undefined; return }
...
onMounted(() => {
  tickHandle = setInterval(() => { now.t = Date.now() }, 1000) as unknown as number
})
